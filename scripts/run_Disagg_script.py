...
for (names, pred_noise), runs, disaggs in product(
    zip(l_names, l_pred_noise), l_runs, l_disaggs
):
    lines.append(
        f"nohup python3 RunSACChargeWorld.py --agent train_6month_sac_pred_noise_{runs}{names}_199 --save-name 6month_sac_disagg_{runs}{names}_{disaggs} --seed {runs} --pred-noise {pred_noise} --years 1 --batch-size 512 --learning-starts 0 --alpha 0.02 --rng-test True --test True --policy-frequency 4 --target-network-frequency 2 --disagg {disaggs} --file-sessions df_elaad_preproc_l6months.csv &"
    )
